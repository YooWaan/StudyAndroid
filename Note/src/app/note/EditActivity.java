...
public class EditActivity extends Activity implements View.OnClickListener {
	...
	public void onClick(View v) {
		...
		try {
			if (id == null) {
				this.getContentResolver().insert(DB.EntryTable.CONTENT_URI, values);
			} else {
				this.getContentResolver().update(ContentUris.withAppendedId(
																			DB.EntryTable.CONTENT_URI,																			Long.parseLong(id)),
												 values, null, null);
			}
			result = RESULT_OK;
